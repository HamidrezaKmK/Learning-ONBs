...
class AtomShufflingField(TimeEvolvingField):
    ...
    Takes a pre-computed time embedding (produced by EulerianIsometry) so
    that the time embedding is shared across all submodules within a step.
    Atom-shuffling (Fourier basis mixing) is handled by EulerianIsometry.
    """
